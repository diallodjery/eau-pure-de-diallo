...
import { deleteInvoice, deleteOperation, loadClients, loadExpenses, loadOperations, loadTeamMembers, deleteTeamMember, saveClient, saveExpense, saveInvoice, saveOperation, saveTeamMember, updateClient, updateInvoice, updateOperation, type Client, type Expense, type Operation, type TeamMember } from "@/lib/operations";
...
function DetailPage({ page, onBack, onAction, onAddMember, onDeleteMember, onEditInvoice, onDeleteInvoice, onDeleteOperation, clients, operations, team, expenses, onExpenseSaved }: { page: Exclude<Page, "home">; onBack: () => void; onAction: (action: Action) => void; onAddMember: () => void; onDeleteMember: (member: TeamMember) => void; onEditInvoice: (operation: Operation) => void; onDeleteInvoice: (operation: Operation) => void; onDeleteOperation: (operation: Operation) => void; clients: Client[]; operations: Operation[]; team: TeamMember[]; expenses: Expense[]; onExpenseSaved: (expense: Expense) => void }) {
  ...
  const saveNewExpense = async () => {
    ...
    if (!expenseCategory.trim() || !Number.isFinite(amount) || amount <= 0) {
      toast("Montant invalide", { description: "Saisissez un montant supérieur à 0 F." });
      return;
    }
    ...
    setExpenseAmount("");
    setExpenseNote("");
    toast("Dépense enregistrée", { description: `${amount.toLocaleString("fr-FR")} F · ${result.source === "firebase" ? "Enregistrée dans Firebase." : "Enregistrée sur cet appareil."}` });
  };
  const shareReceipt = async (operation: Operation) => {
...
export default function Home() {
  ...
  const [editingInvoice, setEditingInvoice] = useState<Operation | null>(null);
  ...
  const saveMember = async () => {
    const normalizedName = memberName.trim().toLowerCase().replace(/\s+/g, " ");
    if (!normalizedName || !Number(memberRate) || Number(memberRate) < 0) return;
    if (team.some((member) => member.name.trim().toLowerCase().replace(/\s+/g, " ") === normalizedName)) {
      toast("Membre déjà enregistré", { description: "Ce nom existe déjà dans votre équipe. Vous pouvez supprimer le doublon depuis Mon équipe." });
      return;
    }
  ...
  const removeMember = async (member: TeamMember) => {
    if (!window.confirm(`Supprimer ${member.name} de l’équipe ?`)) return;
    try {
      await deleteTeamMember(member);
      setTeam((items) => items.filter((item) => item.id !== member.id));
      toast("Membre supprimé", { description: `${member.name} ne sera plus proposé pour les prochaines productions.` });
    } catch (error) {
      console.warn("Membre non supprimé.", error);
      toast("Suppression impossible", { description: "Vérifiez la connexion puis réessayez." });
    }
  };

  const removeInvoice = async (operation: Operation) => {
  ...
  const removeOperation = async (operation: Operation) => {
    if (!window.confirm(`Supprimer cette opération${operation.client ? ` de ${operation.client}` : ""} ? Cette action est irréversible.`)) return;
    try {
      await deleteOperation(operation);
      ...
      const nextOperations = operations.filter((item) => operation.id ? item.id !== operation.id : !(item.createdAt === operation.createdAt && item.type === operation.type && item.client === operation.client));
      ...
      toast("Opération supprimée", { description: "Les données liées ont été recalculées." });
    } catch (error) { console.warn("Opération non supprimée.", error); toast("Suppression impossible", { description: "Vérifiez la connexion puis réessayez." }); }
  };

  const beginEditInvoice = (operation: Operation) => {
    setEditingInvoice(operation);
    setSaleQuantity(String(operation.quantity || 0));
    setSaleUnitPrice(String(operation.unitPrice || 0));
    setSalePaid(String(operation.paid || 0));
  };
  const saveInvoiceEdit = async () => {
    ...
    const paid = Math.min(Math.max(0, Number(salePaid || 0)), total);
    if (quantity <= 0 || unitPrice < 0) { toast("Valeurs invalides", { description: "La quantité doit être supérieure à 0 et le prix ne peut pas être négatif." }); return; }
    const updated = { ...editingInvoice, quantity, unitPrice, amount: total, paid, balanceDue: Math.max(0, total - paid) };
    try {
      await updateOperation(updated);
      await updateInvoice({ operationId: updated.id, clientId: updated.clientId, client: updated.client || "Client", quantity, unitPrice, total, paid, balanceDue: updated.balanceDue, createdAt: updated.createdAt });
      const client = clients.find((item) => item.id === editingInvoice.clientId || item.name === editingInvoice.client);
      if (client && client.id) {
        const updatedClient = { ...client, balance: Math.max(0, (client.balance || 0) - (editingInvoice.balanceDue || 0) + updated.balanceDue), totalPurchased: Math.max(0, (client.totalPurchased || 0) - (editingInvoice.amount || 0) + total) };
        await updateClient(client.id, updatedClient);
        setClients((items) => items.map((item) => item.id === client.id ? updatedClient : item));
      }
      setOperations((items) => items.map((item) => item.id === updated.id ? updated : item));
      setEditingInvoice(null);
      setSaleQuantity("1"); setSaleUnitPrice("225"); setSalePaid("0");
      toast("Facture modifiée", { description: "Le total, le paiement et la dette du client ont été recalculés." });
    } catch (error) { console.warn("Facture non modifiée.", error); toast("Modification impossible", { description: "Vérifiez la connexion puis réessayez." }); }
  };
  ...
  const saveAction = async () => {
    ...
    if (!activeAction || savingAction) return;
    setSavingAction(true);
    if ((activeAction === "sortie" || activeAction === "retour") && !deliveryDriver.trim()) {
      toast("Livreur manquant", { description: "Saisissez le nom du livreur avant d’enregistrer." });
      setSavingAction(false);
      return;
    }
    if (activeAction === "sortie" && (!Number.isFinite(Number(deliveryQuantity)) || Number(deliveryQuantity) <= 0)) {
      toast("Quantité invalide", { description: "Saisissez le nombre de packs remis au livreur." });
      setSavingAction(false);
      return;
    }
    if (activeAction === "retour" && (!Number.isFinite(Number(retours)) || Number(retours) < 0 || Number(retours) > Math.max(0, packsOnRoute))) {
      toast("Retour invalide", { description: "Saisissez un nombre de packs retournés compris entre 0 et les packs en tournée." });
      setSavingAction(false);
      return;
    }
    if (activeAction === "vente" && clientName.trim()) {
      ...
      await saveInvoice({ operationId: operation.operation.id, client: clientName.trim(), quantity: saleQuantityNumber, unitPrice: saleUnitPriceNumber, total, paid, balanceDue, createdAt: new Date().toISOString() });
    ...
    if (activeAction === "retour") { setStock(value => value + Number(retours || 0)); setRetours(""); }
  ...
  if (page !== "home") return <div className="simple-app"><header className="simple-header"><div className="simple-brand"><img className="simple-logo-image" src="/eau-pure-de-diallo-logo.png" alt="EAU PURE DE DIALLO" /><div><strong>EAU PURE</strong><small>DE DIALLO · MA GESTION</small></div></div><div className="simple-header-actions"><button className="simple-profile" onClick={() => setPage("home")}>M</button></div></header><main className="simple-main"><DetailPage page={page} onBack={() => setPage("home")} onAction={setActiveAction} onAddMember={() => setAddMemberOpen(true)} onDeleteMember={(member) => void removeMember(member)} onEditInvoice={beginEditInvoice} onDeleteInvoice={(operation) => void removeInvoice(operation)} onDeleteOperation={(operation) => void removeOperation(operation)} clients={clients} operations={operations} team={team} expenses={expenses} onExpenseSaved={(expense) => setExpenses((items) => [expense, ...items])} /></main>{editingInvoice && <div className="simple-modal-backdrop" onMouseDown={() => setEditingInvoice(null)}><div className="simple-modal" onMouseDown={e => e.stopPropagation()}><div className="simple-modal-head"><div className="modal-action-icon purple"><Receipt size={21} /></div><button onClick={() => setEditingInvoice(null)}><X size={19} /></button></div><p className="modal-kicker">MODIFIER LA FACTURE</p><h2>{editingInvoice.client || "Client"}</h2><p className="modal-help">Modifiez la quantité, le prix ou le montant payé. Le reste sera recalculé.</p><div className="two-inputs"><label>Quantité<input type="number" min="1" value={saleQuantity} onChange={event => setSaleQuantity(event.target.value)} /></label><label>Prix du pack<input type="number" min="0" value={saleUnitPrice} onChange={event => setSaleUnitPrice(event.target.value)} /></label></div><label>Montant payé<div className="simple-input-wrap"><input type="number" min="0" value={salePaid} onChange={event => setSalePaid(event.target.value)} /><span>F</span></div></label><div className="simple-calculation"><span>Nouveau reste à payer</span><strong>{Math.max(0, Number(saleQuantity || 0) * Number(saleUnitPrice || 0) - Number(salePaid || 0)).toLocaleString("fr-FR")} F</strong></div><div className="simple-modal-actions"><button className="cancel-button" onClick={() => setEditingInvoice(null)}>Annuler</button><button className="save-button" onClick={() => void saveInvoiceEdit()}><Check size={17} /> Enregistrer</button></div></div></div>}{addMemberOpen && <div className="simple-modal-backdrop" onMouseDown={() => setAddMemberOpen(false)}><div className="simple-modal" onMouseDown={e => e.stopPropagation()}><div className="simple-modal-head"><div className="modal-action-icon green"><Users size={21} /></div><button onClick={() => setAddMemberOpen(false)}><X size={19} /></button></div><p className="modal-kicker">MON ÉQUIPE</p><h2>Ajouter une personne</h2><p className="modal-help">Son tarif sera conservé sur chaque production.</p><label>Nom<input value={memberName} onChange={event => setMemberName(event.target.value)} placeholder="Ex. Moussa" /></label><label>Commission par pack<div className="simple-input-wrap"><input type="number" min="0" value={memberRate} onChange={event => setMemberRate(event.target.value)} /><span>F / pack</span></div></label><div className="simple-modal-actions"><button className="cancel-button" onClick={() => setAddMemberOpen(false)}>Annuler</button><button className="save-button" onClick={() => void saveMember()}><Check size={17} /> Enregistrer</button></div></div></div>}{activeAction && <div className="simple-modal-backdrop" onMouseDown={closeAction}><div className="simple-modal" onMouseDown={e => e.stopPropagation()}><p className="modal-kicker">NOUVELLE OPÉRATION</p><h2>J’ai vendu à un client</h2><label>Nom du client<input value={clientName} onChange={event => setClientName(event.target.value)} placeholder="Ex. Boutique Alpha" /></label><label>Téléphone<input value={clientPhone} onChange={event => setClientPhone(event.target.value)} placeholder="Ex. 70 00 00 00" /></label><div className="two-inputs"><label>Quantité<input type="number" min="1" value={saleQuantity} onChange={event => setSaleQuantity(event.target.value)} /></label><label>Prix du pack<input type="number" min="0" value={saleUnitPrice} onChange={event => setSaleUnitPrice(event.target.value)} /></label></div><label>Montant payé<div className="simple-input-wrap"><input type="number" min="0" value={salePaid} onChange={event => setSalePaid(event.target.value)} /><span>F</span></div></label><div className="simple-calculation"><span>Reste à payer</span><strong>{Math.max(0, Number(saleQuantity || 0) * Number(saleUnitPrice || 0) - Number(salePaid || 0)).toLocaleString("fr-FR")} F</strong></div><div className="simple-modal-actions"><button className="cancel-button" onClick={closeAction}>Annuler</button><button className="save-button" onClick={() => void saveAction()} disabled={savingAction}>{savingAction ? "Enregistrement…" : <><Check size={17} /> Enregistrer</>}</button></div></div></div>}</div>;
