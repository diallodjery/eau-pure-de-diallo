...
import { loadClients, loadOperations, loadTeamMembers, saveClient, saveInvoice, saveOperation, saveTeamMember, type Client, type Operation, type TeamMember } from "@/lib/operations";
...
export default function Home() {
  ...
  const saveAction = async () => {
    ...
    if (activeAction === "vente" && clientName.trim()) {
      ...
      const result = await saveClient({ name: clientName.trim(), phone: clientPhone.trim(), balance: balanceDue, totalPurchased: total, lastPurchase: new Date().toLocaleDateString("fr-FR") });
      setClients((items) => [result.item, ...items]);
      const operation = await saveOperation({ type: "vente", quantity: saleQuantityNumber, unitPrice: saleUnitPriceNumber, amount: total, paid, balanceDue, client: clientName.trim() });
