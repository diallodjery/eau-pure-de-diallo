...
export async function saveOperation(operation: Omit<Operation, "createdAt">) {
  ...
  try { await addDoc(collection(db, "operations"), { ...payload, ownerId: ownerId(), createdAt: serverTimestamp() }); return { source: "firebase" as const, operation: payload }; }
...
export async function loadOperations() {
  try { const snapshot = await getDocs(userQuery("operations")); const items = snapshot.docs.map((item) => ({ id: item.id, ...item.data(), createdAt: readableDate(item.data().createdAt) })) as Operation[]; return { source: "firebase" as const, items }; }
  catch (error) { console.warn("Lecture opérations Firestore indisponible.", error); return { source: "local" as const, items: localItems<Operation>(LOCAL_OPERATIONS_KEY) }; }
}
